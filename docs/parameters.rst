...
``drift_shape_rank`` and ``drift_shape_nbasis``
-----------------------------------------------
Also specific to this fork, and only used when ``drift_segment_starts`` is set. With ``drift_shape_rank = 0`` (default) drift is constant within each segment. A positive rank adds smooth within-segment drift: each segment learns its own ``drift_shape_rank`` shapes from the data, shared by all depth blocks of that segment, and each block gets its own amount of each shape per segment. The rank can be at most the number of registration blocks, ``2*nblocks - 1``. Each shape is built from ``drift_shape_nbasis`` Gaussian bumps spanning the segment, so fewer bumps give smoother shapes. Use it when the residual diagnostics show drift within segments, for example settling at the start of every session. See the `Learned within-segment drift shape <https://github.com/dimokaramanlis/ChronicKilosort#learned-within-segment-drift-shape>`_ section of the README.
